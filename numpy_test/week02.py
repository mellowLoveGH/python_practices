...
for name,group in grp:
    print(name)
    print(group)
uid = transaction.loc[transaction['UID'] == '100000']
rs = uid[['UID', 'day']]
grp = rs.groupby('day').size()
print(grp)
times = pd.DatetimeIndex(rs['time'])
    grp = rs.groupby([times.hour]).size()
'''
transaction=readData(path1)
...
#grp = groupByID(transaction)


userid = groupByID(transaction)
N = len(userid)
i = 0
while i < 10:
    v = userid[i]
    print(v)
    uid = transaction.loc[transaction['UID'] == v]
    rs = uid[['UID', 'device1']]
    grp = rs.groupby('device1').size()
    print(grp)
    i = i + 1
